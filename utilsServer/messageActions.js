...
const sendMsg = async (userId, msgSendToUserId, msg) => {
  try {
    ...
    let user = await ChatModel.findOne({ user: userId });

    if (!user) {
      user = ChatModel.create({ user: userId, chats: [] });
    }

    // RECEIVER
    let msgSendToUser = await ChatModel.findOne({ user: msgSendToUserId });

    if (!msgSendToUser) {
      msgSendToUser = ChatModel.create({ user: msgSendToUserId, chats: [] });
    }
